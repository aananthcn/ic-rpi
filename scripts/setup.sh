#!/usr/bin/env bash
# setup.sh — Initialize the ic-rpi5 integration project
#
# What this script does:
#   1. Creates /opt/car-ui/{bin,lib,etc} on the host if missing
#   2. Checks and installs host build dependencies
#   3. Registers and clones git submodules (vhal-core, cluster-ui)
#   4. Installs Conan 2.x if not present
#   5. Generates the host Conan profile (profiles/host)
#   6. Calls scan-rpi5-profile.sh to generate the rpi5 Conan profile
...
# ---------------------------------------------------------------------------
# Step 2: Check and install host build dependencies
# ---------------------------------------------------------------------------
HOST_DEPS=(
    git
    cmake
    ...
    python3-venv
    pkg-config
    openssh-client      # needed to SSH into RPi5 for profile scan
    rsync               # needed to deploy to RPi5
    # Qt6 is NOT installed via apt — use the Qt Online Installer (see README.md)
)

check_host_deps() {
    info "Checking host build dependencies..."
    ...
    if [[ ${#missing[@]} -eq 0 ]]; then
        success "All host dependencies are present."
        return
    fi
...
# ---------------------------------------------------------------------------
# Step 5: Generate host Conan profile
# ---------------------------------------------------------------------------
setup_host_profile() {
    info "Generating host Conan profile..."
...
EOF
    ...
    success "Host profile written to profiles/host."
}

# ---------------------------------------------------------------------------
# Step 6: Generate RPi5 Conan profile (via scan script)
# ---------------------------------------------------------------------------
setup_rpi5_profile() {
    local scan_script="${SCRIPT_DIR}/scan-rpi5-profile.sh"

    if [[ ! -f "$scan_script" ]]; then
        warn "scan-rpi5-profile.sh not found at ${scan_script}. Skipping RPi5 profile generation."
        warn "Run ./scripts/scan-rpi5-profile.sh manually once the RPi5 is reachable."
        return
    fi

    info "Running RPi5 profile scan..."
    bash "${scan_script}"
}
...
main() {
    ...
    echo
    echo "Next steps:"
    echo "  Build for host :  ./scripts/build.sh --target host"
    echo "  Build for RPi5 :  ./scripts/build.sh --target rpi5"
    echo "========================================"
}
